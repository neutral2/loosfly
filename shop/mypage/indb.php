...
switch ($_POST[mode]){
	...
	case "taxapp":
		...
		### 주문데이타
		$query = "select m_no, emoney, coupon, enuri, delivery from ".GD_ORDER." where ordno='$_POST[ordno]'";
		$data = $db->fetch($query);

		### 가격 계산
		$goodsnm = '';
		$price = $supply = $surtax = $memberdc = $cnt = 0;
		$query = "select * from ".GD_ORDER_ITEM." where ordno='$_POST[ordno]'";
		$res = $db->query($query);
		while ($item=$db->fetch($res))
		{
			if ( !isset($item[tax]) ) $item[tax] = '1';

			if ( $item[tax] == '1' && $item[istep]<40 ) # 과세
			{
				$price		+= $item[price] * $item[ea];
				$memberdc	+= $item[memberdc] * $item[ea];

				if ( empty($goodsnm) ) $goodsnm = $item[goodsnm];
				$cnt++;
			}
		}

		if($set['tax']['tax_delivery'] == 'y') $price += $data['delivery'];
		...
		### 할인액 계산
		$discount = $memberdc + $data[emoney] + $data[coupon] + $data[enuri];

		### 부가세계산
		if ( ($price - $discount) > 0 )
		{
			$price	-= $discount;
			$supply	= round( $price / 1.1 );
			$surtax	= $price - $supply;
		}
		else $price = 0;
