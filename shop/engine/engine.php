...
$sql = "
				select gg.goodsno,
						...
						gg.sales_range_end
							...
							from ".GD_GOODS." gg
								left join ".GD_GOODS_LINK." gl On(gg.goodsno=gl.goodsno)
									left join ".GD_GOODS_OPTION." ggo On(gl.goodsno=ggo.goodsno)  and go_is_deleted <> '1' and go_is_display = '1'
										where gg.open='1' $where
										group by gg.goodsno
...
switch ($_GET[mode]){
 case "danawa" :
	...
	while($row = $db->fetch($res)){
		...
		fwrite($fp,$row[goodsno].'^'.$cate_f.'^'.$row[goodsnm].'^'.$row[maker].'^'.$img_url.'^'.$goods_url.'^'.$row[price].'^'.$row[reserve].'^'.$row[coupon].'^'.$card[cardfree].'^'.'^'.$row[goodscd].'^'.$row[shortdesc].'^'.$row[regdt].'^'.$d_price.chr(10));
