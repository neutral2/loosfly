...
$combine_menu = array(
	...
	'marketing' => array('marketing','naver','naverCheckout','naverNcash','daumcpc','auctionos','natebasket','keyword','criteo','advertise','interpark','auctionIpay','nateClipping','nateclog','engine','plusCheese','open'),
	...
	for ($i=0,$m=sizeof($keys);$i<$m;$i++){
		...
		if (is_uploaded_file($_FILES[$key][tmp_name][$kk])){
			...
			if (move_uploaded_file($_FILES[$key][tmp_name][$kk],$_dir.$file[$key][name][$kk])) {
				...
				if ($key!="img_s" && $key!="img_i" && $key!="img_c" && $key!="opticon_a" && $key!="opticon_b" && $key!="img_mobile") thumbnail($_dir.$file[$key][name][$kk],$_dirT.$file[$key][name][$kk],45);
...
function naver_goods_diff($goodsno,$ar_update,$class="U") {
	...
	$query = "select category,hidden from ".GD_GOODS_LINK." where goodsno='$goodsno' limit 1";
...
function getPurePhoneNumber($number) {
	...
	return (preg_match('/^([0]{1}[0-9]{1,2}([1-9]{1})?)-?([1-9]{1}[0-9]{2,3})-?([0-9]{4})$/',$number)) ? $number : '';	// 전화번호가 아니라면 빈문자열 리턴.
