...
<!-- 친구에게 상품 알리기 SMS내용-->
<div class=title>친구에게 상품 알리기 SMS내용 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=todayshop&no=8')"><img src="../img/btn_q.gif" border=0 align=absmiddle hspace=2></a></div>
<img src="../img/btn_smstext.gif" align="absmiddle" />
<ul style="width:100%; border:1px solid #cccccc; background:#f7f7f7; padding:5px; margin:5px 0px 5px 0px;">
	<? foreach($spChr as $val) { ?>
	<li style="float:left; border:1px solid #dddddd; width:20px; height:20px; background:#ffffff;" align="center" onClick="insChr(this.innerHTML);" class="hand" onmouseover="this.style.background='#FFC0FF'" onmouseout="this.style.background='#ffffff'"><?=$val?></li>
	<? } ?>
</ul>
<div style="border:solid 1px #D5D5D5; padding:10px;">
	<textarea name="sms" style="width:98%; height:60px;" class="tline" onkeydown="chkLength(this);" onkeyup="chkLength(this);" onchange="chkLength(this);"><?=$data['sms']?></textarea>
	<div class=extext>
		<div>친구에게 상품 알리기SMS발송은 회원만 가능합니다.</div>
		<div>입력한 SMS내용이 80Bytes를 초과하는 경우 여러개의 문자로 발송됩니다. (80Bytes씩 나눠서 전송)</div>
		<div>SMS포인트가 남아 있지 않은 경우 친구에게 상품 알리기 SMS가 발송이 되지 않습니다.</div>
	</div>
...
<div style="text-align:right;font-style:verdana;font-size:12px;"><input id="vLength" type="text" style="width:40px;text-align:right;border:0;font-style:verdana;" value="">/80 Bytes</div>
<script type="text/javascript">chkLength(document.fm.sms);</script>
