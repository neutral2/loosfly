...
$order_query = $db->_query_print('SELECT om.order_no, om.reg_date, om.order_idx, om.mall_cd, om.mall_login_id, om.mall_goods_cd, om.mall_order_no, om.mall_goods_nm, om.order_nm, om.receive_nm, om.settle_price, om.status FROM '.GD_MARKET_ORDER.' om WHERE '.$where.' ORDER BY om.reg_date DESC');

if(!$cfg['orderPageNum']) $cfg['orderPageNum'] = 15;

$result = $db->_select_page($cfg['orderPageNum'], $page, $order_query);
...
function setCmdSelect() {
	...
	for(var i=0; i<len_opt; i++) {
		...
		switch(search_status){
			case '0010' :
				if (send_status_opt[opt_no].value == '0020') {
					opt_no ++;
					continue;
				}
				break;
			case '0020' :
				if (send_status_opt[opt_no].value == '0030') {
					opt_no ++;
					continue;
				}
				break;
...
<tr><td class="rnd" colspan="20"></td></tr>
<?
if(!is_array($result['record']) || empty($result['record'])) { ?>
<tr height="25"align="center">
	<td colspan="9">데이터가 없습니다.</td>
</tr>
<tr><td colspan="20" bgcolor="#E4E4E4"></td></tr>
<? } else {
$res_order = $result['record'];
$ord_idx = ($page - 1) * $cfg['orderPageNum'];
foreach($res_order as $row_order) { 
	$ord_idx ++;
	?>
<tr height="25"align="center">
	<td class="noline"><input type="checkbox" name="chk[]" value="<?=$row_order['order_idx']?>" onclick="iciSelect(this)" <?=$disabled?> class="chk_ordno" /></td>
	<td><?=$ord_idx?></td>
	<td><?=$row_order['reg_date']?></td>
	<td><?=$mall_cd[$row_order['mall_cd']]?><br /><?=$row_order['mall_login_id']?></td>
	<td><a href="javascript:popup('./popup.order.selly.php?ordno=<?=$row_order['order_no']?>',800,600)"><?=$row_order['order_no']?>(<?=$row_order['mall_order_no']?>)</a><br />
	<a href="<?=str_replace('{mall_goods_cd}', $row_order['mall_goods_cd'], $mall_goods_url[$row_order['mall_cd']])?>" target="_blank"><?=$row_order['mall_goods_nm']?></a></td>
	<td><?=$row_order['order_nm']?></td>
	<td><?=$row_order['receive_nm']?></td>
	<td><?=number_format($row_order['settle_price'])?></td>
	<td><?=$order_status[$row_order['status']]['code_nm']?></td>
</tr>
<tr><td colspan="20" bgcolor="#E4E4E4"></td></tr>
<? }
}?>
...
<div align="center" class="pageNavi ver8" style="font-weight:bold">
	...
	<? endif; ?>
	<? if(is_array($pageNavi['page']) && !empty($pageNavi['page'])):
...
<tr>
	<td>마켓상태 일괄처리</td>
	<td>선택한주문
		<select name="send_status" id="send_stauts">
