		...
		if ( empty( $_FILES['file_excel'][name] ) ) $altMsg = 'CSV파일을 선택하지 않으셨습니다.'; // 화일이 없으면
			...
			while ( $data = fgetcsv( $fp, 135000, ',' ) ){
				...
				if($order_no) {
					...
					$send_chk_query = $db->_query_print('SELECT morder_no, send_yn, exchange_send_yn FROM '.GD_MARKET_ORDER.' WHERE order_no=[s]', $order_no);
					...
					if($send_chk['morder_no']) {
						...
						if($upd_res) {
							...
							$arr_delivery['delivery_cd'] = $Recode['deliverycode'];
							$arr_delivery['delivery_no'] = $Recode['deliveryno'];
