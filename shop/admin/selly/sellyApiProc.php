...
switch($mode) {
	...
	case 'sendorder' :
		...
		$res_arr = array();
		$json = new Services_JSON();
		if($res['code']) { $res_arr = $res; $res_arr['order_idx'] = $arr['order_idx']; echo $json->encode($res_arr); exit; }
	...
	case 'linkgoods' :
		...
		$field = 'g.goodsnm, g.origin, g.maker, g.tax, g.delivery_type, g.goods_delivery, g.keyword, g.shortdesc, g.longdesc, g.img_l, g.runout, g.optnm, g.goods_delivery, g.launchdt';
		$field .= ', b.brandnm';
		...
		### 상품옵션 검색 START ###
		$query = $db->_query_print('SELECT * FROM '.GD_GOODS_OPTION.' WHERE goodsno=[s] and go_is_deleted <> \'1\' ', $goods_no);
		$result = $db->_select($query);
		$arr_opt = $result;
		unset($query, $result);
		### 상품옵션 검색 END ###
		...
		### 상품카테고리 검색 START ###
		$cate_query = $db->_query_print('SELECT * FROM '.GD_GOODS_LINK.' WHERE goodsno=[s] order by sort DESC, sno ASC', $goods_no);
		...
		if($res['code'] == '000') {
			...
			$arr_data['code'] = $res['selly_code'];
			$arr_data['msg'] = $res['selly_msg'];
