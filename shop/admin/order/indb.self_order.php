...
switch($mode) {
	...
	case "addGoods" :
		...
		$goodsno	= ($_POST['goodsno'])	? trim($_POST['goodsno'])	: "";
		$opt		= ($_POST['S_opt'])		? trim($_POST['S_opt'])		: "";
		$addopt		= ($_POST['S_addopt'])	? trim($_POST['S_addopt'])	: "";
		$stock		= ($_POST['S_stock'])	? trim($_POST['S_stock'])	: "";
		$memID		= ($_POST['memID'])		? trim($_POST['memID'])		: "";
		$opt		= explode("|||", $opt);
		$addopt		= explode("|||", $addopt);
		$stock		= explode("|||", $stock);

		$addopt_inputable = ($_POST['S_addopt_inputable']) ? trim($_POST['S_addopt_inputable']) : "";
		$addopt_inputable = explode("|||", $addopt_inputable);
		...
		for($i = 0, $m=sizeof($opt); $i < $m; $i++) {

			$_opt = explode("|", str_replace(",", "|", $opt[$i]));
			$_ea = $stock[$i];
			$_addopt = explode(",", $addopt[$i]);
			$_addopt_inputable = explode(",", $addopt_inputable[$i]);

			$cart->addCart($goodsno,array_notnull($_opt),$_addopt,$_addopt_inputable,$_ea,0);
	...
	case "selectMember" :
		...
		$qr = "SELECT email, phone, mobile, zipcode, address, road_address, address_sub, emoney FROM ".GD_MEMBER." WHERE m_id = '$m_id' LIMIT 1";
		$data = $db->fetch($qr);
...
opener.$('m_address').value = '".$data['address']."';
opener.$('m_address_sub').value = '".$data['address_sub']."';
opener.$('m_road_address').value = '".$data['road_address']."';
if(opener.$('m_road_address').value != '') {
	opener.$('m_div_road_address').innerHTML = '".$data['road_address']."';
	opener.$('div_road_address_sub').innerHTML = '".$data['address_sub']."';
} else {
...
";
	...
	case "writeOrder" :
		...
		### 적립금 재계산
			...
			if($cart->totalprice - $discount < 0) $_POST['emoney'] = $cart->totalprice - $_POST['coupon']-$cart->dcprice-$cart->special_discount_amount;
