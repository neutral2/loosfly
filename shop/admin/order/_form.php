...
<tr bgcolor="<?=$bgcolor?>" bg="<?=$bgcolor?>">
	...
	<? if (($item[istep]==41 || ($item[istep]==44 && $item[cyn].$item[dyn]=="nn")) && $naverMileageRecover){ ?><div id="img_recovery"><a href="javascript:recovery(<?=$item[sno]?>)"><img src="../img/btn_return.gif" border=0></a></div><? } ?>
	</td>
</tr>
<?
}
...
<tr>
	...
	<tr>
		...
		<?}?>
		<?if(($order['settlekind'] == 'c' || $order['settlekind'] == 'u') && $order['step'] == 1 && $order['step2'] == 0 && $order['cardtno'] && $order['cdt'] >= date('Y-m-d h:i:s',strtotime("-2 day")) && $order['pgcancel'] != 'r'){?>
...
<tr>
	<td>주문금액</td>
	<td align=right><font class=ver8><b><?=number_format($order->getAmount())?></b></font>원</td>
	<td>&nbsp;</td>
</tr>
<? if ($order['eggFee']){ ?>
<tr>
	<td>소비자피해보상보험</td>
	<td align=right class="blue"><font class=ver8>+ <?=number_format($order['eggFee'])?></font>원</td>
	<td>&nbsp;</td>
</tr>
<? } ?>
<tr>
	<td>할인금액</td>
	<td align=right class="red"><font class="ver8">- <?=number_format($order->getDiscount())?></font>원</td>
	<td><img src="../img/arrow_gray.gif" align=absmiddle><font class=small color=444444>
	<?=implode(' ', $order->getDiscountDetailArray(true))?>
	+ 에누리 <input type=text name=enuri value='<?=$order[enuri]?>' size=6 class='ver81 right' style='color:#0074BA'> 원
	<? if($order[step2] > 40){ ?>&nbsp;<a href="javascript:couponDelPop();"><img src="../img/btn_savedmoney.gif" align=absmiddle></a>
...
<tr>
	<td>배송비</td>
	<td align=right class="blue"><font class=ver8>+ <?=number_format($order->getDeliveryFee())?></font>원</td>
	<td><img src="../img/arrow_gray.gif" align=absmiddle><font class=small color=444444>
	<?=implode(' ', $order->getDeliveryFeeDetailArray(true))?>
	</td>
</tr>
<tr>
	<td>최초결제금액</td>
	<td align=right><font class=ver8><b><?=number_format($order->getSettleAmount())?></b></font>원</td>
	<td><img src="../img/arrow_gray.gif" align=absmiddle><font class=small color=444444>
	주문금액(<?=number_format($order->getAmount())?>원) - 할인금액(<?=number_format($order->getDiscount())?>)
	+ 배송비(<?=number_format($order->getDeliveryFee())?>원)
	<? if ($order['eggFee']){ ?>
	+ 소비자피해보상보험(<?=number_format($order['eggFee'])?>원)
	<? } ?>
	</td>
</tr>
<? if ($order->getCanceledAmount()) { ?>
<tr>
	<td>취소금액</td>
	<td align=right class="red"><font class=ver8>- <?=number_format($order->getCanceledAmount())?></font>원</td>
	<td><img src="../img/arrow_gray.gif" align=absmiddle><font class=small color=444444>
	<?=implode(' ', $order->getCanceledDetailArray('%s(%s원)'))?>
	</td>
</tr>
<tr>
	<td>실결제금액</td>
	<td align=right><font class=ver8><b><?=number_format($order->getRealSettleAmount())?></b></font>원</td>
	<td><img src="../img/arrow_gray.gif" align=absmiddle><font class=small color=444444>
	최초결제금액(<?=number_format($order->getSettleAmount())?>원) - 취소금액(<?=number_format($order->getCanceledAmount())?>원)
	</td>
</tr>
<? } ?>
</table>
...
<dl style="margin:5px;">
	<dt><font color=627dce>※</font> <font class=extext>최초결제금액</font></dt>
	<dd style="padding:0;margin:-13px 0 0 80px;" class="extext">
	- 처음 결제된 금액입니다.<br>
	- 에누리 금액 적용 시 최초 결제금액이 변경 될 수 있습니다.<br>
	</dd>

	<dt><font color=627dce>※</font> <font class=extext>취소금액</font></dt>
	<dd style="padding:0;margin:-13px 0 0 80px;" class="extext">
	- 부분취소 시 쿠폰금액할인, 적립금, 배송비는 취소 금액에 반영되지 않습니다. 이 경우 실결제금액이 마이너스(-)로 표시될수 있습니다.<br>
	- 전체취소 시 쿠폰금액할인, 회원할인, 적립금, 배송비가 자동으로 취소금액에 반영됩니다.
	</dd>

	<dt><font color=627dce>※</font> <font class=extext>실결제금액</font></dt>
	<dd style="padding:0;margin:-13px 0 0 80px;" class="extext">
	- 실결제금액은 최초결제금액에서 취소금액이 차감된 금액입니다.
	</dd>
</dl>
<p>
<?
...
<tr>
	...
	<tr>
		<td>결제종류</td>
		<td><?=$r_settlekind[$order[settlekind]]?><!--<?if($order[settlekind]=='c'){?>&nbsp;<a href='javascript:cardCancel();'>[신용카드취소]</a><?}?>--></td>
	</tr>
	<? if ($order[settlekind]=="a"){ ?>
	<tr>
		<td>입금계좌</td>
