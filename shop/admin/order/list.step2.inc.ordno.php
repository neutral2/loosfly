		...
		<tr height=25 bgcolor="<?=$bgcolor?>" bg="<?=$bgcolor?>" align=center>
			...
			<td align=left>
				...
				<tr>
					...
					<td width="50" align="center"><font class=small1 color=444444><?=$item['ea']?>개</font></td>
				</tr>
			...
			<td><? if ($orderinfo[m_id]) { ?><span id="navig" name="navig" m_id="<?=$orderinfo[m_id]?>" m_no="<?=$orderinfo[m_no]?>"><? } ?><font class=small1 color=0074BA>
			<b><?=$orderinfo[ord_name]?></b><? if ($orderinfo[m_id]){ ?> (<?=$orderinfo[m_id]?>)</font><? if ($orderinfo[m_id]) { ?></span><? } ?>
			<? } ?>
			</td>
