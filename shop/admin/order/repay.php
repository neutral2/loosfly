...
c.settleprice, c.goodsprice, c.delivery, c.coupon, c.emoney, c.memberdc,c.enuri,c.eggFee,c.escrowyn,c.pgcancel,c.pg,d.m_no,d.m_id
";
			...
			if($data['settlekind'] == 'c' || $data['settlekind'] == 'u' ){		// 카드결제 일때만 취소 부분 출력
				if( $repay == $data['settleprice'] ){	// 전체취소 ( 결제금액과 환불금액이 같을 때 )
					if( $data['pgcancel'] == 'y' ){
						...
						echo "<a href=\"javascript:cardSettleCancel('".$data[ordno]."','".$data[sno]."',".$i.")\"><img src='../img/cardcancel_btn.gif' ></a>";
					}
				}else{									// 부분취소
