...
$pg->field = "
a.sno,a.regdt canceldt,a.name nameCancel,a.bankcode,a.bankaccount,a.bankuser,
...
o.settleprice, o.goodsprice, o.delivery, o.coupon, o.emoney, o.memberdc,o.enuri,o.eggFee,o.escrowyn,o.pgcancel,o.inflow,d.m_no,d.m_id,
oi.istep, oi.cyn
";
...
// 카드전체취소
function cardSettleCancel(ordno,sno){
	var obj = document.ifrmHidden;
	if(confirm('카드결제를 취소하시겠습니까?'))	obj.location.href = "cardCancel.php?ordno="+ordno+"&sno="+sno;
}
// 카드부분취소
...
<tr>
	...
	<td>
	<? if ($data[m_id]){ ?><span id="navig" name="navig" m_id="<?=$data[m_id]?>" m_no="<?=$data[m_no]?>"><font class=small1 color=0074BA><?=$data[nameOrder]?></font></span>
	<? } else { ?><font class=small1><?=$data[nameOrder]?></font>
	<? } ?>
	</td>
	<td><font class=small1 color=444444><?=$data[nameCancel]?></td>
	<td><font class=ver7 color=444444><?=$data[cnt]?>/<?=$cnt?></td>
	<td><font class=small1 color=444444><?=$r_settlekind[$data[settlekind]]?></td>
</tr>
<tr>
...
<tr>
	...
	$query = "select sum(remoney), sum(rprice) from ".GD_ORDER_ITEM." a left join ".GD_ORDER_CANCEL." b on a.cancel = b.sno where a.ordno='$data[ordno]'";
	...
	<tr>
		...
		<?
			### 취소처리 부분
			if($data['settlekind'] == 'c' || $data['settlekind'] == 'u' ){		// 카드결제 일때만 취소 부분 출력
				if( $repay == $data['settleprice'] ){	// 전체취소 ( 결제금액과 환불금액이 같을 때 )
					if( $data['pgcancel'] == 'y' ){
						...
						echo "<a href=\"javascript:cardSettleCancel('".$data[ordno]."','".$data[sno]."')\"><img src='../img/cardcancel_btn.gif'></a>";
					}
				}else{									// 부분취소
					if($cancel_info['pgcancel'] != 'r' && $cardPartCancelable === false){
						echo '카드부분취소 지원안됨';
					}else if($cancel_info['pgcancel'] != 'r'){	// 이미 부분취소된 건이 아닐 때
						if( $cfg['settlePg'] == 'inicis' && $data['escrowyn'] != 'n' ){	// 이니시스일 때 에스크로 결제건은 제외
			...
			else if ($data['settlekind'] == 'h' && in_array($data['pg'], array('mobilians'))) {
				if ($repay == $data['settleprice']) {
					...
					echo '휴대폰 결제건으로<br/>부분취소가 불가능합니다.';
				}
			}
			else{
				echo "카드결제건이 아닙니다.";
