...
while ($data=$db->fetch($res)){
	...
	if ($data['attach']) {
		$data[contents] = ($data['attach'] ? '<img src="/shop/data/review/RV'.sprintf("%010s", $data[sno]).'" name="rv_attach_image[]" border="0">'."\n" : '').$data[contents];
...
<tr height="25" align="center" onmouseover="this.style.background='#F7F7F7'" onmouseout="this.style.background=''">
	...
	<td width="70"><a href="../../goods/goods_view.php?goodsno=<?=$data['goodsno']?>" target="_blank"><?=goodsimg($data['img_s'],40,"style='border:1px solid #efefef'",1)?></a></td>
