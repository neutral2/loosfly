...
if( $_POST[catnm] && ($_POST[mode] == 'addEvent' || $_POST[mode] == 'modEvent') ){
	...
	if( !$_POST[category] ){
		$db->query("insert into ".GD_CATEGORY." set category='".$next_category."',catnm='".$_POST[catnm]."',hidden='1',sort=unix_timestamp()");
		$cate_ins = true;
	}else{
...
if( $_POST[catnm] && ($_POST[mode] == 'addEvent' || $_POST[mode] == 'modEvent') ){
	...
	while($tmp = $db->fetch($res)){
		$timestamp = time()+$i;
		$db->query("insert into ".GD_GOODS_LINK." set goodsno='$tmp[goodsno]',category='$next_category',hidden='$hidden',sort=-'".$timestamp."'");
		$i=$i+10;
	}
}
