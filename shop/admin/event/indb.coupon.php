...
if($_POST[mode] == 'applyAdd' || $_POST[mode] == 'applyMod'){
	...
	if($_POST[smsyn]){
		if($_POST[membertype] == '0'){
			$squery = "select mobile from ".GD_MEMBER." where mobile";
		}else if($_POST[membertype] == '1'){
			$squery = "select a.mobile from ".GD_MEMBER." a left join ".GD_MEMBER_GRP." b on a.level=b.level where b.sno='$_POST[member_grp_sno]' and a.mobile";
		}else if($_POST[membertype] == '2'){
			$squery = "select mobile from ".GD_MEMBER." where m_no in (".implode(',',$_POST[m_ids]).") and mobile";
		}
		if($squery){
			...
			while($data = $db->fetch($res)){
				...
				if ($sms->send($_POST[msg],$mobile,$_POST[callback])){
					 $sms->log($_POST[msg],$mobile,$case,1);
				}
