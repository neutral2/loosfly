...
if($sno){
	...
	if($arCoupon['goods_apply'] == 'limited'){
		$query = "
				SELECT coupon.*,goods.goodsnm,options.price
...
<tr>
	<td>적용상품</td>
	<td>
	<label class="noline"><input type="radio" name="goods_apply" class="goods_apply"  value="all" />전체상품에 적용</label>
	<label class="noline"><input type="radio" name="goods_apply" class="goods_apply" value="limited" />특정 카테고리 및 상품선정</label>
	<div id="goods_apply_id">
	...
	<script>new categoryBox('goods[]',4,'','');</script>
	<input type="text" name="search_goods" onkeydown="return go_list_goods('goods')">
	<a href="javascript:list_goods('goods')"><img src="../img/i_search.gif" align=absmiddle></a>
	<a href="javascript:view_goods('goods')"><img src="../img/i_openclose.gif" align=absmiddle></a>

	</div>
	<div id="obj_goods" class="box1"><iframe id="ifrm_goods" style="width:100%;height:100%" frameborder=0></iframe></div>
	<div id="obj2_goods" class="box2 scroll" onselectstart="return false" onmousewheel="return iciScroll(this)">
		<div class="boxTitle">- 등록된 상품 <font class="small" color="#F2F2F2">(삭제하려면 더블클릭)</font></div>
		<table id="tb_goods" class="tb">
		<col width="50">
	<?
	if ($arGoods) foreach ($arGoods as $v):
		if($v['goodsno']):
	?>
		<tr onclick="spoit('goods',this)" ondblclick="remove_('goods',this)" class="hand">
			...
			<input type="hidden" name="e_goods[]" value="<?=$v['goodsno']?>">
			</td>
		</tr>
	<?
		endif;
	...
	<div id="goodsX" style="font:0"></div>
	</div>
	<div>
	<script>react_goods('goods');</script>
	</div>
	</div>
