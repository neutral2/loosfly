...
elseif($mode=='modify') {
	...
	if($_FILES['design_head_file']['error'] == UPLOAD_ERR_OK) {
		move_uploaded_file($_FILES['design_head_file']['tmp_name'],SHOPROOT.'/data/attendance/'.$attendance_no.'_head.jpg');
	}
	if($_FILES['design_stamp_upload']['error'] == UPLOAD_ERR_OK) {
		move_uploaded_file($_FILES['design_stamp_upload']['tmp_name'],SHOPROOT.'/data/attendance/'.$attendance_no.'_stamp.jpg');
	}
...
elseif($mode=='reserve') {
	...
	foreach($result as $v) {
		...
		if($smsyn=='1') {
			...
			if($sms->send($msg,$v['mobile'],$callback)) {
