...
$db_table .= '						JOIN gd_member_grp AS e ON (d.level = e.level) ';

$where[] = 'a.sno= '.$_GET['sno'];
if ($_GET['skey'] && $_GET['sword']){
	if ( $_GET['skey'] == 'resno' ){
		$tmp = str_replace( "-", "", $_GET['sword'] );
		$where[] = "( d.resno1=md5('" . substr( $tmp, 0, 6 ) . "') and d.resno2=md5('" . substr( $tmp, 6, 7 ) . "') )";
	}
...
$pg -> field = " a.sno, a.coupon_name, b.regdt as down_date, d.name, d.m_id, e.grpnm, c.regdt, p.number ";
...
<tr>
	...
	<option value="resno" <?=$selected['skey']['resno']?>> 주민번호 </option>
