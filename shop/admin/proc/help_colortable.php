...
//        if (opener.chwin)        statusInit();
        ...
        document.all['SWATCHES'].onmousemove=swatchStatus;
        document.all['SWATCHES'].onmousedown=swatchSelect;
}
function test(){
...
function targetSelect(){
        ...
        if (obj.tagName=="BUTTON" && obj.className=='outset'){
                document.all[color.target].className='outset';
                obj.className='inset';
                color.target=obj.id;
        }
}
...
function swatchStatus(){
        ...
        document.all['SAMPLE'].style.backgroundColor=hexValue;
}
...
function adjustStatus(){
        ...
        crdY=changeScale((event.y-obj.offsetTop),obj.clientHeight,255);
        if (color.mode <=2){
...
<tr><td height=40></td></tr>
  <TR><TD style="padding-left:2px" valign=top class=text1>선택 <INPUT  size=7 value=0 name=text><?if($_GET['iconidx']  != '' && $_GET['target']  != '' ){?><div align="center" style="padding-top:5px"><a href="javascript:goodsicon()"><img src="../img/btn_color_select.gif" align="absmiddle" border="0"></a><?}?></TD>
    </TR>
