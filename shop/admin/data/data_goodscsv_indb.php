			...
			//if($data[$FieldNm[goodsno]]){ //널체크 by birdmarine 2007.1.30
				...
				foreach ( $etcRecode as $key => $value ){ // 별도처리 필드
					...
					if ( $key == 'goodscate' ){ // 상품분류
						...
						foreach ( $tmp as $k => $v ) {
							$v = trim($v);
							$tmp[ $k ] = str_pad($v, ceil(strlen($v) / 3 ) * 3, '0', STR_PAD_LEFT);
						}
						...
						foreach ( $tmp as $category ){
							...
							if ( $cnt < 1 ){
								...
								$db->query("
									...
									set goodsno='" . $Recode['goodsno'] . "', category='" . $category . "', sort=-unix_timestamp()
									".(count($sortList) ? ', '.implode(', ', $sortList) : ''));
							}
						}
						...
						foreach ( $value as $recode ){
							...
							list( $opt1, $opt2, $price, $consumer, $supply, $reserve, $stock ) = explode( "^", $recode );
							...
							if ( $cnt < 1 ) $db->query( "insert into ".GD_GOODS_OPTION." set goodsno='" . $Recode['goodsno'] . "', opt1='" . $opt1 . "', opt2='" . $opt2 . "', price='" . $price . "', consumer='" . $consumer . "', supply='" . $supply . "', reserve='" . $reserve . "', stock='" . $stock . "', link='" . $link . "', go_is_deleted = '0', go_is_display = '1'" );
							else if ( $cnt == 1 ) $db->query( "update ".GD_GOODS_OPTION." set price='" . $price . "', consumer='" . $consumer . "', supply='" . $supply . "', reserve='" . $reserve . "', stock='" . $stock . "', link='" . $link . "', go_is_deleted = '0', go_is_display = '1' where goodsno='" . $Recode['goodsno'] . "' and opt1='" . $opt1 . "' and opt2='" . $opt2 . "' and go_is_deleted <> '1'" );
