					...
					while ( $data = $db->fetch($res) ) {
						...
						foreach ( $fields as $key => $arr ) {
							...
							if ( $key == 'goodscate' ) {
								...
								$tmp = array();
								$t_res = $db->query( "select opt1, opt2, price, consumer, supply, reserve, stock from ".GD_GOODS_OPTION." where goodsno = '" . $data['goodsno'] . "' AND go_is_deleted <> '1' order by link desc, sno" );
