	...
	{ // CSV 파일의 전체 항목을 읽어 DB에 insert
		...
		while ( $data = fgetcsv( $fp, 135000, ',' ) ){ // 데이타 저장
			...
			foreach ( $FieldNm as $key => $dataIdx ){ // Recode 배열 구성
				...
				if ( in_array( $key, array( 'resno1', 'birth_year', 'birth', 'busino', 'emoney', 'marridate', 'cnt_login', 'cnt_sale', 'sum_sale' ) ) ) $Recode[$key] = preg_replace( '/[^0-9]/', '', $Recode[$key] ); // 숫자만 저장
				...
				if ( in_array( $key, array( 'resno1' ) ) && $_POST[chkpass] ) $Recode[$key] = md5( $Recode[$key] ); // md5 저장

				if ( in_array( $key, array( 'resno2' ) ) && $_POST[chkpass] ) $Recode[$key] = md5( $Recode[$key] ); // md5 저장
