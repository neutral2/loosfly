...
switch ($mode){
	...
	case "sms_auto":
		...
		$cfg['smsRecall']	= @implode("-",$_POST['smsRecall']);
	...
	case "sms_address_add_by_excel":	// 엑셀 일괄 등록
		...
		if ($file['error'] == 0 && $file['size'] > 0) {
			...
			for ($i=3, $max=sizeof($arRow);$i<$max;$i++) {
				...
				// 휴대폰 번호 체크
				if (! preg_match('/^([0]{1}[0-9]{1,2})-?([1-9]{1}[0-9]{2,3})-?([0-9]{4})$/',$arRow[$i][$fields['sms_mobile']])) {
	...
	case "send_sms":
		...
		switch ($_POST[type]){
			...
			case "3":				// 회원 선택 대상
				$to_tran= $r_smsType[$_POST['type']];
				$where	= "m_no IN (".implode(",",$_POST['chk']).") AND sms='y' AND mobile <> '' ";
			...
			case "6":
				...
				$query	= "select mobile from ".GD_MEMBER." $where";
				$res	= $db->query($query);
				$total	= $db->count_($res);
				break;
			case "7":
				$to_tran= $r_smsType[$_POST['type']];
				$query	= "SELECT sms_mobile FROM ".GD_SMS_ADDRESS;
				$res	= $db->query($query);
				$total	= $db->count_($res);
				break;
		}
		...
		// LMS
			...
			$msg = "LMS 발송건수 : ".number_format(array_sum($num))."건 \\n ------------------- \\n 성공 : ".number_format($num[success])." / 실패 : ".number_format($num[fail]);
		}
	...
	case "delete":
		...
		foreach ($_POST[chk] as $v){
			...
			// 탈퇴로그 저장
			list( $m_id, $name, $resno1, $resno2 ) = $db->fetch("select m_id, name, resno1, resno2 from ".GD_MEMBER." where m_no='$v'");
			$db->query("insert into ".GD_LOG_HACK." ( m_id, name, resno1, resno2, actor, ip, regdt ) values ( '$m_id', '$name', '$resno1', '$resno2', '0', '" . $_SERVER['REMOTE_ADDR'] . "', now() )" );
	...
	case "modify":
		...
		if ( $mod_resno == 'Y' ) $mod_query .= "resno1 = md5('" . $n_resno[0] . "'), resno2 = md5('" . $n_resno[1] . "'),";
		if ( $mod_sex == 'Y' ) $mod_query .= "sex = '$sex',";
		...
		if ($cfg['ssl'] == "1") {
			if($cfg['ssl_type'] == "free") { //무료보안서버사용
				$sslcheck = $sitelink->link('admin/member/info.php' ,"regular");
			} else {//유료보안서버사용
				$sslcheck = $sitelink->link('admin/member/info.php?' ,"regular");
			}
