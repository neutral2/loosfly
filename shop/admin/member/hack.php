...
if ($_GET['skey'] && $_GET['sword']){
	if ( $_GET['skey']== 'resno' || $_GET['skey']== 'all' ){
		$tmp = str_replace( "-", "", $_GET['sword'] );
		$resno_where = "( resno1=md5('" . substr( $tmp, 0, 6 ) . "') and resno2=md5('" . substr( $tmp, 6, 7 ) . "') )";
	}

	if ( $_GET['skey']== 'all' ){
		$where[] = "( concat( m_id, name, ip ) like '%".$_GET['sword']."%' or $resno_where )";
	}
	else if ( $_GET['skey']== 'resno' ) $where[] = $resno_where;
