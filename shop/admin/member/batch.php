...
list ($total) = $db->fetch("select count(*) from ".GD_MEMBER); # 총 레코드수
...
function veiwFunc(fObj)
{
	...
	if (func == 'sms'){ 
	  ...
	  fObj.msg.removeAttribute("required");
	}
...
function chkFuncForm(fObj)
{
	...
	if (func == 'sms'){
		if(fObj.sms_type[0].checked == true && fObj['sms_msg'].value == ''){
			alert( fObj['sms_msg'].getAttribute('msgR') );
			fObj['sms_msg'].focus();
			return false;
		} else if(fObj.sms_type[1].checked == true && fObj['lms_msg'].value == ''){
			alert( fObj['lms_msg'].getAttribute('msgR') );
			fObj['lms_msg'].focus();
			return false;
		}
	}
...
<tr valign=top>
	...
	<option value="query">검색한 회원 모두에게<?=($pg->recode[total]?"({$pg->recode[total]}명)":"")?>
	...
	<div><label for="func4"><input type=radio name=func id=func4 onClick="veiwFunc(this.form)" value="sms" <?=$checked['func']['sms']?>>SMS를 발송합니다.</label></div>
	</b></td>
</tr>
</table>
</div>
...
<? $total = is_object($pg) ?  $pg->recode['total'] : $total ?>
