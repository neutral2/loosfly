...
<tr height=50>
	<td>적립금 적립기준</td>
	<td>
	<div><input type=radio name="emoney_standard" value="0" class=null <?=$checked['emoney_standard'][0]?>> 판매가
	<input type=radio name="emoney_standard" value="1" class=null <?=$checked['emoney_standard'][1]?>> 결제가</div>
<div class="extext_t">* 적립금 적립기준을 "판매가"로 선택 시 적립금/쿠폰/배송비 적용 전 기본 판매가를 기준으로 적림금이 적립됩니다.<br>
* 적립금 적립기준을 "결제가"로 선택 시 적립금/쿠폰/배송비 적용 후 구매자가 실제 결제한 금액을 기준으로 적림금이 적립됩니다.</div>
	</td>
</tr>
</table>
</div>
...
<div class=title>금액절삭관리<span>금액절삭관리는 적립금, 할인쿠폰의 적용으로 발생되는 결제금액 끝자리 단위를 관리하기 위함입니다</span> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=basic&no=4')"><img src="../img/btn_q.gif" border=0 align=absmiddle></a></div>
<table class=tb>
<col class=cellC><col class=cellL>
<tr>
	<td>기본자리수</td>
	<td>
	<input type=text name=cut size=1 value="<?=$set['cut']?>" option="regNum" label="기본자리수" onkeydown="onlynumber()" class=rline> 자리 이하 절삭 처리
	<div class="extext" style="padding:5 0 0 5">ex) 0자리 : 절삭안함, 1자리: 10원단위, 2자리: 100원단위, 3자리: 1000원단위</div>
	<div class="extext" style="padding:0 0 0 3">공백은 2자리로 자동 설정됩니다  (숫자로만 입력하세요)</div>
	</td>
</tr>
</table>
