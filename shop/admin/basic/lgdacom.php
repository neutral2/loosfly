...
if($cfg['settlePg']!="lgdacom") $pg = array(); //pg타입체크
...
if ($set['use']['u']) $checked['u'] = "checked";
...
function chkSettleKind(){
	...
	// 중국 카드 관련
	if (document.getElementsByName('set[use][u]')[0].checked == true )	{
		$$('input[name="pg[cup_id]"]','input[name="pg[cup_mertkey]"]').each(function(obj){
			obj.style.background = "#ffffff";
			obj.readOnly = false;
		});
	}
...
function chkFormThis(f){
	...
	if (document.getElementsByName('set[use][u]')[0].checked == true)
	{
		if (! document.getElementsByName('pg[cup_id]')[0].value) {
			alert('LG U+ CUP ID는 필수항목입니다.');
			return false;
		}

		if (! document.getElementsByName('pg[cup_mertkey]')[0].value) {
			alert('LG U+ CUP mertkey는 필수항목입니다.');
			return false;
		}

		if(!chkPgCUPid()){
			alert('LG U+ CUP ID가 올바르지 않습니다.');
			return false;
		}
...
// 중국 은련카드 관련 함수
var oldCUPId = "<?php echo $pg['cup_id'];?>";

function chkPgCUPid() {
	var obj = document.getElementById('pgcupid');
	var pattern = /^(CUP_go_|CUP_fp_|CUP_fd_)/;
	if(pattern.test(obj.value) || (oldCUPId == obj.value && oldCUPId)){
		return true;
	}else if(obj.value){
		return false;
	}
...
<div class="title">중국 은행연합 카드결제 설정</div>
<div style="padding-top:15px"></div>
<table border="1" bordercolor="#e1e1e1" style="border-collapse:collapse" width="100%">
<col class="cellC"><col class="cellL">
<tr>
	<td>결제수단 설정</td>
	<td class="noline">
	<input type="checkbox" name="set[use][u]" <?=$checked['u']?> onclick="chkSettleKind();"> CUP
	&nbsp;&nbsp;&nbsp;<font class="extext"><b>(반드시 LG U+ PG사와 계약 후 체크하세요.)</b></font></td>
</tr>
<tr>
	<td class="ver8"><b>LG U+ CUP ID</td>
	<td>
	<div style="float:left"><input type="text" name="pg[cup_id]" class="lline" value="<?=$pg[cup_id]?>" onkeyup="chkPgCUPid()" onblur="chkPgCUPid()" id="pgcupid"></div>
	<div style="clear:both" class="extext">LG U+ CUP ID는 'CUP_go_, CUP_fp_, CUP_fd_' 로 시작되는 아이디만 입력 가능합니다. (단, 기존 입력값은 무방합니다)</div>
	<!--div class="extext">고도몰에서 신청하지 않은, 다른 값으로 시작되는 아이디의 경우에는 개별 승인신청을 하셔야 합니다.</div-->
	</td>
</tr>
<tr>
	<td class="ver8"><b>LG U+ CUP mertkey</td>
	<td>
	<input type="text" name="pg[cup_mertkey]" class="lline" value="<?=$pg['cup_mertkey']?>">
	</td>
</tr>
</table>
...
<div id="MSG04">
<table cellpadding="1" cellspacing="0" border="0" class="small_ex">
<tr><td><img src="../img/icon_list.gif" align="absmiddle">중국은행 연합카드는 LG U+의 일반 전자결제 서비스를 이용하는 고객만 신청이 가능합니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">신청 후 발급 받은 중국은행 연합카드 전용 ID, Key를 입력하시길 바랍니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">부분취소, 할부 서비스는 지원되지 않습니다.</td></tr>
</table>
</div>
...
<tr>
	<td>결제 수단</td>
	<td class="noline">
	<input type="checkbox" name="escrow[c]" <?=$checked['method']['c']?>> 신용카드
