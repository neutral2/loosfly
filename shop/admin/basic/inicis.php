...
function iniescrowUSE() {	
	if(document.getElementsByName('escrow[type]')[1].checked){ //이니에스크로를 선택했다면
...
<tr>
	...
	<input type=checkbox name=set[use][h] <?=$checked[h]?> onclick="chkSettleKind()"> 핸드폰
...
<div id="MSG03">
	...
	<tr><td>이니시스에서 제공하는 하나은행 에스크로 결제수단을 방문자(소비자)에게 제공하기 위해서</td></tr>
	<tr><td>이니시스에서 <b>메일로 받으신 압축파일을 풀어서 Escrow ID와 Escrow Key File 3개를 입력</b>하신후 본 페이지 하단의 저장버튼을 클릭해 주세요.</td></tr>
	<tr><td>아직 이니시스와 하나은행 에스크로를 계약 하지 않으셨다면</td></tr>
	<tr><td style="padding-left:10">①계약서류를 우편으로 이니시스에 접수후</td></tr>
	<tr><td style="padding-left:10">②<a href="http://www.hanaescrow.com/new/apply/1/1/index.jsp" target="_blank"><font color="#ffffff"><u>하나에스크로 사이트(http://www.hanaescrow.com)에서 공인인증서로 전자서명</u></font></a></td></tr>
	</table>
	</div>
	<div id="MSG03_2">
	<table border=0 cellpadding=1 cellspacing=0 border=0 class="small_ex">
...
<tr>
	...
	<input type=radio name=escrow[use] value="N" <?=$checked[escrow]['use'][N]?> onclick="chkEscrow()"> 사용안함<span style="padding-left:15" id="hana_ment"><font class=extext><b>(이니시스의 하나은행 에스크로를 계약하셨다면 사용으로 체크하세요)</b></font></span><span style="padding-left:15" id="ini_ment"><font class=extext><b>(이니시스의 이니에스크로를 계약하셨다면 사용으로 체크하세요)</b></font></span>
	</td>
</tr>
<tr>
	<td>에스크로종류</td>
	<td class="noline">
	<input type="radio" name="escrow[type]" value="HANA" <?=$checked['type'][HANA]?> onclick="iniescrowUSE()"> 하나에스크로
	<input type="radio" name="escrow[type]" value="INI" <?=$checked['type'][INI]?> onclick="iniescrowUSE()"> 이니에스크로
	</td>
</tr>
...
<tr>
	<td>기관 설정</td>
	<td class=noline>
	<input type=radio name=escrow[comp] value="PG" <?=$checked[escrow][comp][PG]?>> 해당 PG사 에스크로
	<input type=radio name=escrow[comp] value="hanaBank" <?=$checked[escrow][comp][hanaBank]?> disabled> 하나은행 에스크로
	</td>
</tr>
-->
...
<tr>
	<td>결제 수단</td>
	<td class="noline">
	<input type=checkbox name=escrow[c] <?=$checked[method][c]?> disabled> 신용카드
	<input type=checkbox name=escrow[o] <?=$checked[method][o]?> disabled> 계좌이체
	<input type=checkbox name=escrow[v] <?=$checked[method][v]?>> 가상계좌<span style="padding-left:15" id="hana_ment2"><font class=extext><b>(이니시스의 하나은행 에스크로는 가상계좌 결제수단에 지원 됩니다)</b></font></span><span style="padding-left:15" id="ini_ment2"><font class=extext><b>(이니시스의 이니에스크로는 가상계좌 결제수단에 지원 됩니다)</b></font></span>
	</td>
</tr>
<tr>
	<td>사용 금액</td>
