	...
	if ((($phone = $formatter->get($row['phone'],'dial','-')) !== false) && ($sms->smsPt > 0)) {
		...
		foreach($smsMsg as $msgContent){
			if ($sms->send($msgContent,$phone,$cfg['smsRecall'])) {
				...
				$sms->update();
				$sms->log($msgContent,$row['name']."(".$phone.")",$type,"1");
			}
		}
		unset($smsMsg);
	}
