...
<tr>
	<td>문자 알림 사용 <input type="checkbox" name="smsYn" id="smsYn" style="border:0px;" value="1"<?=($purchaseSet['smsYn'] == 1) ? " checked" : ""?> /></td>
	<td>
		재고 <input type="text" name="smsStock" id="smsStock" value="<?=$purchaseSet['smsStock']?>" size="3" /> 개 미만
		<select name="cp1" id="cp1">
<?
	$cpExchangeNo = array("010", "011", "016", "017", "018", "019");
	for($i = 0, $imax = count($cpExchangeNo); $i < $imax; $i++) {
?>
			<option value="<?=$cpExchangeNo[$i]?>"<?=($cpExchangeNo[$i] == $purchaseSet['cp1']) ? " selected" : ""?>><?=$cpExchangeNo[$i]?></option>
<?
	}
?>
		</select> -
		<input type="text" name="cp2" id="cp2" value="<?=$purchaseSet['cp2']?>" size="4" maxlength="4" /> -
		<input type="text" name="cp3" id="cp3" value="<?=$purchaseSet['cp3']?>" size="4" maxlength="4" />
		&nbsp; &nbsp; <span class="small" style="color:#6D6D6D;">SMS 알림 기능사용 (SMS 포인트충전이 되어있어야 가능합니다)</span>
	</td>
</tr>
</table>
