...
<? } ?>
	...
	<tr>
		...
		<tr><td height="31" background="../img/sms_bottom.gif" align="center"><font class="ver8" color="262626"><input name="stockedSMSLen" id="stockedSMSLen" type="text" style="width:20px;text-align:right;border:0;font-size:8pt;font-style:verdana;" value="0">/80 Bytes</td></tr>
		</table>
	...
	<table class="tb">
		...
		<tr>
			...
			<td class="noline">
				<p><input type="radio" name="msgOpt" value="fix" id="fix" <? if($stocked_noti_cfg['msgOpt'] == "fix"){ echo "checked"; } ?> /><label for="fix">단문(80byte) 고정</label><br /><span style="color:#6D6D6D; margin-left: 20px">전송되는 문자의 총 길이가 80Bytes를 초과시, 상품명과 옵션명이 각각 10Bytes로 축소되며</span><br /><span style="color:#6D6D6D; margin-left: 20px">축소된 이후 에도 80Bytes 초과 내용은 잘려서 전송 됩니다.</span></p>
				<p style="margin-top:-10px"><input type="checkbox" name="shortGoodsNm" value="y" id="shortGoodsNm" style="margin-left:50px;" <? if($stocked_noti_cfg['shortGoodsNm'] == "y"){ echo "checked"; } ?> /><label for="shortGoodsNm">상품정보 짧게 표시</label><br /><span style="color:#6D6D6D; margin-left: 70px">80Bytes 기준에 맞추어 상품명과 옵션명이 최소 한 글자까지 축소되어 전송될 수 있습니다.</span></p>
			</td>
		</tr>
		<tr>
			<td class="noline">
				<p><input type="radio" name="msgOpt" value="separate" id="separate" <? if($stocked_noti_cfg['msgOpt'] == "separate"){ echo "checked"; } ?> /><label for="separate">장문(80byte 이상) 분할전송</label><br /><span style="color:#6D6D6D; margin-left: 20px">80Bytes 이상일 경우 SMS 발송건수는 2건 이상으로 나누어 전송됩니다.</span><br /><span style="color:#6D6D6D; margin-left: 20px">장문(80Bytes 이상) 분할전송으로 설정하여도 80Bytes를 넘지 않을 경우, 1건 으로 발송됩니다.</span></p>
			</td>
		</tr>
	</table>
