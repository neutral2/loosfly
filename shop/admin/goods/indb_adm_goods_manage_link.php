...
switch ($mode){
	...
	case "link":
		...
		foreach ($_POST['chk'] as $goodsno){
			...
			if (!$cnt) {
				...
				foreach (array_keys($maxSortIncrease) as $category) $goodsSort->increaseSortMax($category);
				$db->query("insert into ".GD_GOODS_LINK." set goodsno='{$goodsno}',category='{$sCategory}',hidden='{$hidden}',sort=-unix_timestamp()".(count($sortList) ? ', '.implode(', ', $sortList) : ''));
			}
	...
	case "move":
		...
		foreach ($_POST['chk'] as $goodsno){
			...
			foreach (array_keys($maxSortIncrease) as $category) $goodsSort->increaseSortMax($category);
			$db->query("update ".GD_GOODS_LINK." set category='{$sCategory}',hidden='{$hidden}'".(count($sortList) ? ', '.implode(', ', $sortList) : '')." where goodsno='{$goodsno}' and category='{$_POST[category]}'");
			if ($_POST['isToday'] == 'Y') $db->query("update ".GD_GOODS." set regdt=now() where goodsno='{$goodsno}'");
		}
		break;
	...
	case "copyGoodses":
		...
		foreach ($_POST['chk'] as $goodsno){
			...
			foreach (array_keys($maxSortIncrease) as $category) $goodsSort->increaseSortMax($category);
			$db->query("update ".GD_GOODS_LINK." set category='{$sCategory}',hidden='{$hidden}'".(count($sortList) ? ', '.implode(', ', $sortList) : '')." where goodsno='{$cGoodsno}' and category='{$_POST[category]}'");
