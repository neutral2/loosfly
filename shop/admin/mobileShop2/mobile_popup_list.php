...
function open_change(mpopup_no, before, after) {
	...
	if(before == '0' && after == '1') {
		...
		var obj = eval("document.all.sel_open_"+mpopup_no);
...
<tr height=25>
	...
	<td>
		<select name="sel_open_<?=$row_popup['mpopup_no']?>" onChange="open_change('<?=$row_popup['mpopup_no']?>','<?=$row_popup[open]?>',this.value)">
