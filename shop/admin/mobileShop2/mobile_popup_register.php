...
function chkOpenType(){
	if(document.all.open_type[1].checked) document.getElementById('time_set').style.display='block';
	else document.getElementById('time_set').style.display='none';
}

function chkPopupType() {
	if(document.all.popup_type[1].checked) {
	...
	<td >
		<div style="position:relative; height:25px;">
			<div style="position:absolute;">
				<label class="noline"><input type="radio" name="open_type" value="0" onclick="chkOpenType();" <?=$checked['open_type']['0']?> required="required" /> 계속</label>
				<label class="noline"><input type="radio" name="open_type" value="1" onclick="chkOpenType();" <?=$checked['open_type']['1']?> required="required" /> 기간설정</label>
			</div>
...
<tr>
	...
	<td>
			<div style="height:25px;">
				<label class="noline"><input type="radio" name="popup_type" value="0" onclick="chkPopupType();" <?=$checked['popup_type']['0']?> required="required" /> 이미지 업로드</label>
				<label class="noline"><input type="radio" name="popup_type" value="1" onclick="chkPopupType();" <?=$checked['popup_type']['1']?> required="required" /> 텍스트(에디터)입력</label>
			</div>
