...
// 메타태그 변수를 storage에 저장 상품 상세 페이지에서 global로 사용하기 위함
Clib_Application::storage()->set(meta_title, $meta_title);
Clib_Application::storage()->set(meta_keywords, $meta_keywords);
...
if($cfg[custom_landingpage] > 1 && !preg_match( "/main\/intro*/", $key_file) && !preg_match( "/member\/find*/", $key_file) && !preg_match( "/member\/join*/", $key_file) && !preg_match( "/member\/login*/", $key_file) && !preg_match( "/proc\/popup_zipcode*/", $key_file) && !preg_match( "/member\/change*/", $key_file) && !preg_match( "/proc\/popup_address*/", $key_file) ){
	...
	if ($cfg[custom_landingpage] == 2 && !Clib_Application::session()->isAdult() && !$sess) {	// 성인 or 회원
		header('location:'.$cfg[rootDir].'/main/intro_adult.php' . ($_SERVER['QUERY_STRING'] ? '?'.$_SERVER['QUERY_STRING'] : ''));
	}
	elseif ($cfg[custom_landingpage] == 3 && !$sess) {	// 회원
		header('location:'.$cfg[rootDir].'/main/intro_member.php' . ($_SERVER['QUERY_STRING'] ? '?'.$_SERVER['QUERY_STRING'] : ''));
	}
}
...
$customHeader = '';
// SNS JS 파일
$customHeader .= '<script type="text/javascript" src="../lib/js/sns.js"></script>';
