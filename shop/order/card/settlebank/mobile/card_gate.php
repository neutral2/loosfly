...
<!-- 결과처리를 위한 파라미터 -->
<input type="hidden" name="PNoteUrl" value="http://<?=$_SERVER['SERVER_NAME']?><?=$cfg['rootDir']?>/order/card/settlebank/mobile/card_return.php"><!--결제성공시 노티전송받을 url -->
<input type="hidden" name="PNextPUrl" value="http://<?=$_SERVER['SERVER_NAME']?><?=$cfg['rootDir']?>/order/card/settlebank/mobile/pay_rcv.php"><!--결제성공/오류시 호출될 url-->
