...
<!-- VISA3D 결제 -->
<input type="hidden" name="P_NEXT_URL" value="http://<?=$_SERVER['HTTP_HOST']?><?=$cfg['rootDir']?>/order/card/inicis/mobile/card_return.php<?php echo '?ordno='.$_POST['ordno'].'&settlekind='.$_POST['settlekind']; ?>">

<!-- ISP 결제 -->
<input type="hidden" name="P_NOTI_URL" value="http://<?=$_SERVER['HTTP_HOST']?><?=$cfg['rootDir']?>/order/card/inicis/mobile/vacctinput.php">
<input type="hidden" name="P_RETURN_URL" value="http://<?=$_SERVER['HTTP_HOST']?><?=$cfgMobileShop['mobileShopRootDir']?>/ord/order_return_url.php?ordno=<?=$_POST['ordno']?>">
