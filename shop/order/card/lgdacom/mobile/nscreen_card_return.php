...
if ($naverNcash->useyn == 'Y') {
	...
	if ($ncashResult === false) {
		msg('네이버 마일리지 사용에 실패하였습니다.', $order_fail_page.'?ordno='.$_POST['LGD_OID'], 'parent');
		exit();
	}
}
...
if (forge_order_check($HTTP_POST_VARS['LGD_OID'],$HTTP_POST_VARS['LGD_AMOUNT']) === false) {
	msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$order_fail_page.'?ordno='.$HTTP_POST_VARS['LGD_OID'],'parent');
	exit();
}
...
if($isAsync=='Y') {	//비동기 ISP
	...
	$configPath = $_SERVER['DOCUMENT_ROOT'].$cfg['rootDir']."/conf/lgdacom"; //LG유플러스에서 제공한 환경파일("/conf/lgdacom.conf,/conf/mall.conf") 위치 지정.
...
// DB 처리
	...
	if($oData['step'] > 0 || $oData['vAccount'] != '' || !strcmp($LGD_RESPCODE,"S007")){		// 중복결제
		...
		$goUrl=$order_end_page."?ordno=$ordno&card_nm=$card_nm";
	}else{	//결제실패
