...
$query = sprintf("select mb.mobile, mb.email, mb.name, mb.m_id, otp.token, otp.otp, otp.expire, otp.auth from ".GD_OTP." as otp inner join ".GD_MEMBER." as mb on otp.m_id = mb.m_id where otp.m_id = '%s' AND otp.token > '' AND otp.token = '%s'", $db->_escape($_POST['m_id']), $db->_escape($_POST['token']));
...
if ($_POST['mode'] == 'send') {
	...
	switch ((int) $_GET['type']) {
		case 1: // 이메일 전송
			...
			break;
		case 2: // 휴대폰 전송
			if (($_mobile = $sf->get($_mb['mobile'], 'dial', '-')) !== false) {
				...
				if ($sms->send($msg,$_mobile,$cfg['smsRecall'])){
					$sms->log($msg,$_mobile,'',1);
				}
