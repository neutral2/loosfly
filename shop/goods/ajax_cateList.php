...
// 쿼리 생성 및 실행
	$query = "
SELECT
	DISTINCT L.goodsno, L.category, G.img_l, G.goodsnm, O.price
FROM
	".GD_GOODS_LINK." AS L
...
ORDER BY L.sort";
	...
	if(!$page) {
		...
		list($temp_cnt) = $db->fetch("SELECT COUNT(sort) AS cnt FROM ".GD_GOODS_LINK." WHERE sort < $this_sort AND category LIKE '$category%'");
