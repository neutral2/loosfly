...
if( $_GET['searched'] == 'Y' ){
	...
	if ($_GET[skey] && ($_GET[sword] || $_GET['hid_sword'])){
		switch ($_GET[skey]){
//			case "all": $key = "concat( b.keyword, b.goodsnm, b.goodscd, b.maker, if(d.brandnm is null,'',d.brandnm) )"; break;
			case "all": $key = "concat( b.keyword, b.goodsnm, b.shortdesc, b.goodscd, b.maker, if(d.brandnm is null,'',d.brandnm) )"; break;
	...
	while ($data=$db->fetch($res)){
		...
		### 카테고리
		if ($category == '')
			list($data['category']) = $db->fetch("select category from ".GD_GOODS_LINK." where hidden=0 and goodsno='{$data[goodsno]}' limit 1");
		else
