	...
	function makeList($type, $data) {
		...
		switch($type) {
			case "b" : // 상품 기본 정보 - basic
				...
				if($data['price'] == 'y') {
				// 최대가격, 최소가격
					list($maxPrice, $minPrice) = $db->fetch("SELECT MAX(b.price), MIN(b.price) FROM ".GD_GOODS_LINK." AS a INNER JOIN ".GD_GOODS_OPTION." AS b ON a.goodsno = b.goodsno AND b.link = 1 and go_is_deleted <> '1' and go_is_display = '1' WHERE a.category LIKE '".$this->category."%';");
