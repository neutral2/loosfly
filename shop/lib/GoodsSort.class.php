...
class GoodsSort
{
		...
		$result = $this->_dbo->query('
			...
			INNER JOIN (
				...
				FROM (
					...
					WHERE __gl.category LIKE "'.$category.'%"
					GROUP BY __gl.goodsno
					ORDER BY __gl.'.$sortField.' ASC, __gl.sort ASC
				) AS _gl,
				(SELECT @SEQUENCE := 0) AS _sequence
			) AS seqmap
			ON gl.goodsno = seqmap.goodsno AND gl.category LIKE "'.$category.'%"
	...
	function applyModified($category, $modifiedSortSet, $modifiedOpenSet)
	{
		...
		if ($categoryInfo['sort_type'] === 'MANUAL') {
			...
			foreach ($modifiedSortSet as $goodsno => $sort) {
				$result = $this->_dbo->query('UPDATE gd_goods_link SET '.$sortField.'='.$sort.' WHERE goodsno='.$goodsno.' AND category LIKE "'.$category.'%"');
	...
	function movePageSelection($category, $selectedSortSet, $currentPage, $targetPage, $limit, $position)
	{
		...
		$sortField = $this->getSortField($category);
		$result = $this->_dbo->query('UPDATE gd_goods_link SET '.$sortField.'='.$sortField.$spacing.' WHERE category LIKE "'.$category.'%" AND '.$sortField.' BETWEEN '.$sortStart.' AND '.$sortEnd);
