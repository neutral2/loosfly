...
class Board Extends Page
{
	...
	{
		GLOBAL $bdPrnType,$bdUserDsp,$bdAdminDsp,$bdIpAsterisk,$bdTitleCChk,$bdTitleSChk,$bdTitleBChk,$bdListImgSizeW,$bdListImgSizeH;
	...
	function _view()
	{
		GLOBAL $bdAdminDsp,$bdIpAsterisk,$bdUserDsp;
	...
	function setContents($contents,$html,$code=0)
	{
		...
		if ($code){
			...
			if (!strstr($contents,"[:이미지1:]") && $oDiv[0] && eregi("(gif|jpg|bmp|png)$",$oDiv[0])){
				$contents = "<div><img src=\"download.php?id=".$this->id."&no=".$this->data['no']."&div=0&mode=1\" border=\"0\" /></div><br>".$contents;
			}
