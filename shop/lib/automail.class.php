...
class automail {
	...
	function _assign_tpl($ordno) {
		...
		$query="SELECT a.nameOrder,a.email,a.goodsprice,a.settleprice,a.settlekind,a.zipcode,a.address,a.nameReceiver,a.phoneReceiver,a.deliverycode,a.delivery,b.deliveryno,b.deliveryurl
		...
		$row[goodsprice]=number_format($data[goodsprice]);//상품합계금액
		$row[delivery]=number_format($data[delivery]);//배송비
		$row[totalprice]=number_format($data[goodsprice]+$data[delivery]);	//총 주문금액
		$row[settleprice]=number_format($data[settleprice]);//결제금액
		
		$row[zipcode]=$data[zipcode];	//우편번호
