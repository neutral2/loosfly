...
class Clib_Helper_Front_Goods_Abstract extends Clib_Helper_Abstract
{
	...
	public function getGoodsCollectionArray(Clib_Collection_Goods_Abstract $goodsCollection, $category = null)
	{
		$goodsCollection = $this->setGoodsCollectionAttributes($goodsCollection, $category);
	...
	public function setPrevAndNextGoodsNavigationHtml(Clib_Model_Goods_Abstract $goodsModel, Clib_Model_Category_Category $categoryModel)
	{
		...
		list($currentSort) = Clib_Application::database()->fetch("SELECT " . $sortField . " FROM " . GD_GOODS_LINK . " WHERE goodsno = '" . $goodsModel->getId() . "' AND category LIKE '" . $categoryModel->getId() . "%'");
		list($prevGoodsId) = Clib_Application::database()->fetch("SELECT L.goodsno FROM " . GD_GOODS_LINK . " AS L LEFT JOIN " . GD_GOODS . " AS G ON L.goodsno = G.goodsno WHERE L." . $sortField . " < $currentSort AND L.category LIKE '" . $categoryModel->getId() . "%' AND G.open = '1' ORDER BY " . $sortField . " DESC LIMIT 1");
		list($nextGoodsId) = Clib_Application::database()->fetch("SELECT L.goodsno FROM " . GD_GOODS_LINK . " AS L LEFT JOIN " . GD_GOODS . " AS G ON L.goodsno = G.goodsno WHERE L." . $sortField . " > $currentSort AND L.category LIKE '" . $categoryModel->getId() . "%' AND G.open = '1' ORDER BY " . $sortField . " ASC LIMIT 1");
	...
	protected function setGoodsAttributes(Clib_Model_Goods_Abstract $goodsModel, Clib_Model_Category_Category $categoryModel)
	{
		...
		// 코디상품 설정 로드
		$goodsModel['setGoodsConfig'] = $setGoodsConfig['setconnection'];
	...
	public function getGoodsViewUrl($goodsModel, $categoryModel = null)
	{
		...
		if ($goodsModel->getUseOnlyAdult() && ! Clib_Application::session()->canAccessAdult()) {
			$url = '../member/auth_adult.php?returnUrl=' . urlencode($url);
		}
	...
	public function getGoodsViewUrlMobile($goodsModel, $categoryModel = null)
	{
		...
		// 성인
		if ($goodsModel->getUseOnlyAdult() && ! Clib_Application::session()->canAccessAdult()) {
			$url = '../intro_adult.php?returnUrl=' . urlencode($url);
		}
		else if ($categoryModel instanceof Clib_Model_Category_Category && $categoryModel->hasLoaded()) {
			if (Clib_Application::session()->getMemberLevel() < $categoryModel['level'] && $categoryModel['level_auth'] == 3) {
				$url = 'javascript:msg_back();';
