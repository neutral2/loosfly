...
class Clib_Collection_Admin_Goods extends Clib_Collection_Goods_Abstract
{
	...
	public function setCategoryFilter($value, $wildcard = true)
	{
		if ($value != '') {
			if ($wildcard) {
				$this->addFilter('goods_link.category', Clib_Application::database()->wildcard($value, 1), 'like');
			}
			else {
				$this->addFilter('goods_link.category', $value);
			}
		}
	...
	public function setSoldoutFilter($value)
	{
		if ($value === '0') {
			// 품절 상품 제외
			$this->addExpressionFilter("!( goods.runout = 1 OR ( goods.usestock = 'o' AND goods.totstock < 1))");
		}
		else if ($value === '1') {
			// 품절 상품만
			$this->addExpressionFilter(" ( goods.runout = 1 OR ( goods.usestock = 'o' AND goods.totstock < 1))");
