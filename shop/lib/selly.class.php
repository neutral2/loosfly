...
class selly {
	...
	function makeGoods($row) {
		...
		list($row['market_price'], $row['sale_price'], $row['buy_price']) = $db->fetch("SELECT consumer, price, supply FROM gd_goods_option WHERE goodsno = '".$row['goodsno']."' AND price > 0  and go_is_deleted <> '1' and go_is_display = '1' ORDER BY price ASC LIMIT 1");
...
", "", $row['longdesc']))."</desc>\n";
	...
	function ajaxGoods($goodsno="") {
		...
		// 선택된 상품 읽어오기
		$db = &$GLOBALS['db'];
		$query = "SELECT G.*, L.category, B.brandnm FROM gd_goods AS G LEFT JOIN gd_goods_link AS L ON L.goodsno = G.goodsno LEFT JOIN gd_goods_brand AS B ON B.sno = G.brandno WHERE G.goodsno = '$goodsno'";
		$rs = $db->query($query);
		$row = $db->fetch($rs);
...
class sellyRec {
	...
	function makeGoodResult() {
		...
		for($i = 0, $imax = count($this->reqData['g']); $i < $imax; $i++) {
			...
			$db->query("INSERT INTO gd_goods_link SET goodsno='".$tmpThisSeq."', category='".$this->reqData['g'][$i]['category']."', hidden='0', sort=-UNIX_TIMESTAMP()"); // 카테고리 등록
