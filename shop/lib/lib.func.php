...
function getDeliveryMode($param){
	...
	if($param['mode'] === 0) {
		...
		if($set['delivery']['deliveryOrder']) {
			...
			// 결제가 기준
			// (상품가 - 회원할인 - 쿠폰할인 - 적립금(단, 적립금으로 주문시 결제금액에 적립금 주문금액 비포함 설정일때만 적립금을 제함)
			$delivery_criteria = $cart->goodsprice - $cart->dcprice - $cart->coupon - ($set['emoney']['emoney_delivery'] == 1 ? $param['emoney'] : 0);
		}
	...
	if($param['zipcode']){
		...
		if($set['delivery']['area_deli_type'] != '1'){
			$arr = explode('|',trim($set['delivery']['overZipcode']));
			foreach($arr as $k => $v){
				$idx = count($rzip);
				$arr1 = array_unique(explode(',',$v));
				foreach($arr1 as $v2)if($v2)$rzip[$idx][] = $v2;
			}
			if(!$set['delivery']['overAdd']){
				$tmpAdd = explode("|",$set['delivery']['over']);
				foreach($tmpAdd as $v){
				...
				$over = explode("|",$set[delivery][overAdd]);
			}

			$head = substr($param['zipcode'],0,3);

			$param['zipcode'] = preg_replace('/[^0-9]/','',$param['zipcode']);
			$param['zipcode'] = sprintf('%s-%s', substr($param['zipcode'],0,3), substr($param['zipcode'],3,3));
			$tmp = Core::loader('Zipcode')->get( array('keyword' => $param['zipcode'], 'where' => 'zipcode' ) )->current();
			$tmpSido = $tmp['sido'];
			$tmpGugun = $tmp['gugun'];
			...
			$over = explode('|',trim($set['delivery']['overAddZip']));
			$zip = str_replace('-','',$param['zipcode']);
	 ...
	 */
	$r_goods = array();
	$extra_fee; // 위에서 계산된 추가 배송비는 아래에서 연산됨.
	...
	for ($i=0,$item_cnt = sizeof($items);$i<$item_cnt;$i++) {
		...
		switch((int)$_item['delivery_type']) {
			...
			case 1:	## 무료배송상품
				...
				$extra_fee = $extra_fee + $_extra_fee;
				break;
			...
			case 5:	## 수량별 배송비 (배송비가 수량에 비례하여 커짐)
				...
				$extra_fee = $extra_fee + ($_extra_fee * $_item['ea']);
				break;
		}
	...
	if($param['mode'] === 0 && !empty($member_group)) {
		...
		if( $tmp_deliveryfee_chk === true) {
			$delivery['type'] = '무료';
			$delivery['extra_price'] = $delivery['price'] = ($set['delivery']['add_extra_fee'] == '1' || !isset($set['delivery']['add_extra_fee'])) ? $extra_fee : 0;
	...
	*/
	...
	if($ctype_cnt > 0) {
		...
		$delivery['price'] = $default_delivery_policy[2] + $extra_fee;
		$delivery['extra_price'] = $extra_fee;
		...
		$delivery['default_price'] = $delivery['price'];

		if ($ctype_cnt === $item_cnt) return $delivery;
	}
	...
	if ($ctype_cnt > 0 && ($seperate_delivery['prepay_total'] > 0 || $seperate_delivery['prepay_max'] > 0)) {
		...
		$delivery['price'] = $delivery['price'] + $extra_fee;
		$delivery['extra_price'] = $extra_fee;
		...
		switch ((int)$set['delivery']['goodsDelivery']) {
			...
			case 1:			// 상품별 배송비와 기본배송비 중 더 큰 배송비로 합니다.
				$delivery['price'] = ($delivery['price'] < $seperate_delivery['prepay_max']) ? $seperate_delivery['prepay_max'] : $delivery['price'];
				break;
	...
	if($ctype_cnt > 0) {
		...
		if ($ctype_cnt === $item_cnt || $set['delivery']['freeDelivery'] == 1) {
			$delivery['extra_price'] = $delivery['price'] = ($set['delivery']['add_extra_fee'] == '1') ? $extra_fee : 0;
			$delivery['freeDelivery'] = 1;
			$delivery['type'] = '무료';
			$delivery['msg'] = number_format($delivery['price']).' 원';
...
function sendSmsCase($case,$mobile)
{
	...
	if ($send_c && $mobile){
		...
		if ($sms->send($msg_c,$mobile,$GLOBALS['cfg']['smsRecall'])){
			$sms->log($msg_c,$mobile,$case,1);
		}
	}
	...
	if ($send_a && $GLOBALS['cfg']['smsAdmin']){
		...
		if ($sms->send($msg_a,$GLOBALS['cfg']['smsAdmin'],$GLOBALS['cfg']['smsRecall'])){
			 $sms->log($msg_a,$GLOBALS['cfg']['smsAdmin'],$case,1);
		}
	}
	...
	if ($send_m && $GLOBALS['cfg']['smsAddAdmin']){
		...
		foreach ($smsAddAdmin as $key=>$val){
			if ($sms->send($msg_a,$val,$GLOBALS['cfg']['smsRecall'])){
				$sms->log($msg_a,$val,$case,1);
			}
		}
	}
...
function sendSmsStock($ordno) {
	...
	for($i = 0; $data = $db->fetch($res); $i++) {
		...
		if($purchaseSet['smsStock'] > $tmpStock && $checkStockSmsLog == 0) {
			...
			$sPhone = ($phone1 && $phone1 != "--") ? str_replace("-", "", $phone1) : str_replace("-", "", $phone2);
			$msg_p = "[매진예정]
".$data['goodsnm']."
$comnm
남은수량 {$tmpStock}개";
...
### 문자열 길이만큼 자르기
function gd_str_split($msg, $len=80)
{
	$len = strlen($msg);
	$buf = '';
	$str = '';
...
function setGoodslinkHide($category, $hidden,$mobile=0)
{
	...
	if ($hidden == 1 || getCateHideCnt($category) > 0) $db->query("update ".GD_GOODS_LINK." set {$hiddenFieldName}='1' where category like '$category%'");
	else {
		$res = $db->query("select category, {$hiddenFieldName} from ".GD_CATEGORY." where category like '$category%' order by category");
		while ($data=$db->fetch($res)){
			if ( $cateHidden[ substr($data[category],0,-3) ] == 1 ) $data[$hiddenFieldName] = 1;
			$cateHidden[$data[category]] = $data[$hiddenFieldName];
			$db->query("update ".GD_GOODS_LINK." set hidden='".$data[$hiddenFieldName]."' where category='$data[category]'");
		}
	}
}
...
function chkCancel($ordno,$arr){
	...
	if($pre[step] > 3 && $pre[m_no]){
		...
		$query = "select OI.reserve, OI.ea ,LE.memo from ".GD_ORDER_ITEM." AS OI INNER JOIN ".GD_LOG_EMONEY." AS LE ON OI.ordno = LE.ordno where OI.ordno='$ordno' and OI.sno in (".implode(',',$arr[sno]).") and OI.reserve_status='NORMAL'";
		$rs = $db->query($query);
		$reserve = 0;
		while ($row = $db->fetch($rs,1)) {
			if ($row['memo'] == '구매완료로 인해 구매적립금 적립') {
				$reserve += ($row['reserve'] * $row['ea']);
			}
		}
...
### 쿠폰정보
function getCouponInfo($goodsno,$goodsprice='',$mode=''){
	$Goods = Core::loader('Goods');
...
function chk_memberdc_exc($member,$goodsno) {
	...
	if($member['excate']){
		...
		foreach($arr as $cate){
			list($excnt) = $db->fetch( "select count(*) from ".GD_GOODS." a left join ".GD_GOODS_LINK." b on a.goodsno=b.goodsno where a.goodsno='$goodsno' and b.category like '".$cate."%'");
			$tcnt += $excnt;
		}
...
function restore_coupon($ordno){
	global $db;
	//쿠폰 복원
	list($applysno, $downloadsno) = $db->fetch("select applysno, downloadsno  from ".GD_COUPON_ORDER." where ordno='$ordno'");
...
function forge_order_check($ordno, $settleprice = 0)
{
	...
	// 주문 체크
	list($chk_order, $chk_price)	= $db->fetch("SELECT ordno,settleprice FROM ".GD_ORDER." WHERE ordno='".$ordno."'");
...
function getordno() {
	...
	$db->query("LOCK TABLES ".GD_ORDER_TEMP." WRITE");
...
function chkOpenYn($items,$msg="C",$action)
{
	switch($msg) {
...
function rtnOpenYn($items, $msg = null) {
	...
	if(is_object($items)) {	// cart object
		foreach ($items->item as $k=>$item){
			setNotAvailbleGoodsName($item[goodsno], $msg, $goodsname);
		}
	}
	else if(is_array($items)){	//세트상품일경우(배열로넘어옴)
		foreach($items as $val) {
			if(array_key_exists('goodsno', $val) && $val[goodsno]){
				setNotAvailbleGoodsName($val[goodsno], $msg, $goodsname);
			}
			else {
				setNotAvailbleGoodsName($val, $msg, $goodsname);
			}
		}
	}
	else{
		setNotAvailbleGoodsName($items, $msg, $goodsname);
	}
...
// 쿠폰정보 모바일샵
function getCouponInfoMobile($goodsno,$goodsprice='',$mode=''){
	$Goods = Core::loader('Goods');
	$arCategory = $Goods->get_goods_category($goodsno);
...
function getMailOrderData($ordno) {
	...
	$queryOrder="SELECT a.ordno,a.nameOrder,a.phoneOrder,a.mobileOrder,a.nameOrder,a.email,a.goodsprice,a.settleprice,a.settlekind,a.zipcode,a.address,a.nameReceiver,a.phoneReceiver,a.mobileReceiver,a.deliverycode,a.delivery,b.deliveryno,b.deliveryurl,a.memo,a.deli_msg
