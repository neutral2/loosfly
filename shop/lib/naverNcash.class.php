...
class naverNcash{
	...
	function naverNcash($isAdmin = false){
		...
		$this->testerIPList = array('111.91.139.55','58.72.239.56','61.36.175.','211.233.50.','211.233.51.','115.88.179.','111.91.140.58','182.162.206.167');
		if($_COOKIE['NA_MI'])
		{
	...
	**/
	...
	function payment_approval($ordno, $isPaid){
		...
		$use_yn = $db->fetch("select oldordno from " .GD_ORDER. " where ordno ='".$ordno."'");
		if( $use_yn['oldordno'] || !in_array($paymentInfo['save_mode'],array('ncash','both'))) return;
		...
		$ncash_post['isMember'] = ($sess) ? "Y" : "N";	// 가맹점 회원여부 ( 회원 Y , 비회원 N )
	...
	function payment_approval_cancel($ordno,$TxId=null){
		...
		**/
		...
		$res = $this->init("https://".$this->api_url."/v2/partner/".$this->api_id."/payment/".$_COOKIE['reqTxId']."/abort",http_build_query($ncash_post),1);
	...
	function category_name($goodsno){
		...
		$goods_category = $db->fetch("select category from `gd_goods_link` where `goodsno` = $goodsno order by sno limit 1");
