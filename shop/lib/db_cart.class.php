...
class Cart extends old_Cart
{
	...
	function getItems()
	{
		...
		while ($data = $this->db->fetch($rs,1)) {
			...
			if ($data[addno]) {
				...
				$_addopt_value  = explode(',',$data['c_addno_value']);
	...
	function addCart($goodsno,$opt,$addopt,$addopt_inputable,$ea)
	{
		global $sess;
		...
		$addno_value = implode(',', $r_addopt_value);
		...
		$query = "SELECT COUNT(uid) FROM ".GD_CART." WHERE uid = '".$uid."' AND goodsno = '".$goodsno."' AND optno = '".$optno."' AND addno='".$addno."' AND is_direct = ".$this->isDirect."  AND is_buy = 0";
	...
	function chkStock($goodsno,$opt1='',$opt2='',$ea=1,$mode='')
	{
		...
		if(!$goodsnm) $this->msg("일치하는 상품이 없습니다.",-1);
		...
		if ($todaygoods == 'y') {
			...
			if ($_startdt > 0 && $_startdt > $_now) {
				//echo '안시작해서 못삼';
				$this->msg("{$goodsnm} 상품은 {$startdt} 부터 구매 가능합니다.",$goodsurl);
			}

			if ($_enddt > 0 && $_enddt < $_now) {
				//echo '끝나서 못삼';
				$this->msg("{$goodsnm} 상품은 판매마감 되었습니다.",$goodsurl);
			}
		...
		if($ea < $min_ea) {
			if($mode == 'add' || $this->isDirect){
				$this->msg("{$goodsnm} 상품의 최소구매수량은 {$min_ea}개 입니다.",$goodsurl);
			}else{
				$this->msg("{$goodsnm} 상품의 최소구매수량은 {$min_ea}개 입니다.",'../goods/goods_cart.php');
			}
		}
		else if($max_ea > 0 && $ea > $max_ea) {
			if($mode == 'add' || $this->isDirect){
				$this->msg("{$goodsnm} 상품의 최대구매수량은 {$max_ea}개 입니다.",$goodsurl);
			}else{
				$this->msg("{$goodsnm} 상품의 최대구매수량은 {$max_ea}개 입니다.",'../goods/goods_cart.php');
			}
		}

		if( basename($_SERVER['PHP_SELF']) != "cartMove_proc.php" ){
			if ($runout) $this->msg("{$goodsnm}은 품절된 상품입니다",$goodsurl);
			if ($usestock && $ea > $stock){
				if ($stock>0){
					...
					if($mode == 'add' || $this->isDirect){
					    $this->msg("{$goodsnm} 상품의 잔여 재고는 {$stock}개입니다",$goodsurl);
					}else{
						$this->msg("{$goodsnm} 상품의 잔여 재고는 {$stock}개입니다",'../goods/goods_cart.php');
					}
