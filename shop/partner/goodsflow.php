...
function __result($result, $msg='',$failed=null) {
	...
	if ($msg)
		$xml .= '<Msg>'.iconv('euc-kr','utf-8',$msg).'</Msg>'."\r\n";

	if (!empty($failed) && is_array($failed)) {

		$xml .= '<Failed>'."\r\n";
		foreach($failed as $fail) {
			$xml .= '<UniqueCd>'.$fail['UniqueCd'].'</UniqueCd>'."\r\n";
		}
		$xml .= '</Failed>'."\r\n";
	}
...
$postdata = isset($_POST['invoice_print_data']) ? stripslashes($_POST['invoice_print_data']) : '';

$xml = new StrucXMLParser();
$xml ->parse($postdata);
$res = $xml->parseOut();

$result = (array)$res['DTNPRINTLIST'][0]['child']['DTNPRINT'];
...
foreach ($result as $dtnPrint) {
	...
	// UniqueCd
	$UniqueCd = $_tmp['UNIQUE_CD'][0]['data'];

	// 택배사 코드
	$deliveryno = $_gf_delivery_company_map[strtoupper($_tmp['TRANS_CD'][0]['data'])];

	// 송장 번호
	$deliverycode = $_tmp['SHEET_NO'][0]['data'];
...
__result( (empty($queues) ? 'SUCCESS' : 'PARTIAL'),'',$queues);
?>