<?php /* Template_ 2.2.7 2013/11/01 18:21:58 /www/loosfly_godo_co_kr/shop/data/skin/loosfly/proc/popup_coupon.htm 000007377 */ ?>
...
<!--
<?php if(is_array($TPL_R1=($GLOBALS["loop"]))&&!empty($TPL_R1)){$TPL_I1=-1;foreach($TPL_R1 as $TPL_V1){$TPL_I1++;?>
...
<?php if(is_array($TPL_R1=($GLOBALS["loop"]))&&!empty($TPL_R1)){$TPL_S1=count($TPL_R1);$TPL_I1=-1;foreach($TPL_R1 as $TPL_V1){$TPL_I1++;?>
	<tr height=40>
		<td>
...
<?php if($TPL_V1["goodsnms"]){?><div><button onClick="view_goods(<?php echo $TPL_I1?>,<?php echo $TPL_S1?>);">적용상품보기</button></div><?php }else{?>-<?php }?>
		...
		<div><!-- <?php if(is_array($TPL_R2=$TPL_V1["goodsnms"])&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?> --><?php echo $TPL_V2?><br/><!-- <?php }}?> --></div>
		</div>
		</td>
...
<?php }}?>
	...
	<div style="padding:10px" align=center><a href="javascript:window.close()"><img src="/shop/data/skin/loosfly/img/common/btn_confirm_red.gif"></a></div>
	</td>
</tr>
