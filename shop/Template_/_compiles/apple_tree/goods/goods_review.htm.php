<?php /* Template_ 2.2.7 2012/11/14 17:43:56 /www/loosfly_godo_co_kr/shop/data/skin/apple_tree/goods/goods_review.htm 000008089 */ 
...
<tr>
	<td><img src="/shop/data/skin/apple_tree/img/common/title_review.gif" border=0></td>
</tr>
<TR>
...
<tr>
	...
	<tr>
		<td><img src="/shop/data/skin/apple_tree/img/common/search.gif" border=0></td>
		...
		<tr>
			...
			</select><input type="text" NAME="sword" value="<?php echo $_GET['sword']?>" size=47 class="linebg"><input type="image" src="/shop/data/skin/apple_tree/img/common/btn_search.gif" align=absmiddle>
			</td>
		</tr>
		</table>
		</td>
...
<select onchange="this.form.sort.value=this.value;this.form.submit()" style="font:8pt 돋움"><?php if(is_array($TPL_R1=$TPL_VAR["lstcfg"]["sort"])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_K1=>$TPL_V1){?><option value="<?php echo $TPL_K1?>" <?php echo $GLOBALS["selected"]["sort"][$TPL_K1]?>><?php echo $TPL_V1?><?php }}?></select>
<select onchange="this.form.page_num.value=this.value;this.form.submit()" style="font:8pt 돋움"><?php if(is_array($TPL_R1=$TPL_VAR["lstcfg"]["page_num"])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?><option value="<?php echo $TPL_V1?>" <?php echo $GLOBALS["selected"]["page_num"][$TPL_V1]?>><?php echo $TPL_V1?>개씩 정렬<?php }}?></select>
</div>
...
<?php if($TPL_V1["type"]!='Q'&&$TPL_V1["notice"]== 0){?>
				<TD rowspan=2 width="27" valign="top" style="padding-top:3px;"><img src="/shop/data/skin/apple_tree/img/common/ico_a.gif"></TD>
<?php }?>
				<TD style="padding-top:5"><?php if($TPL_V1["notice"]== 0){?><span style="font-weight:bold;"><?php echo $TPL_V1["goodsnm"]?></span> <a href="<?php echo url("goods/goods_view.php?")?>&goodsno=<?php echo $TPL_V1["goodsno"]?>"><img src="/shop/data/skin/apple_tree/img/common/btn_goodview2.gif" align=absmiddle></a><?php }?></TD>
			</TR>
...
<?php if(is_array($TPL_R2=array_fill( 0,$TPL_V1["point"],''))&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?>★<?php }}?>
<?php }?>
	</td>
</tr>
</table>
...
<?php if($TPL_V1["authreply"]=='Y'){?>
	<a href="javascript:;" onclick="popup_register( 'reply_review', '<?php echo $TPL_V1["goodsno"]?>', '<?php echo $TPL_V1["sno"]?>' );"><img src="/shop/data/skin/apple_tree/img/common/btn_reply.gif" border="0" align="absmiddle"></a>
<?php }?>
<?php if($TPL_V1["authmodify"]=='Y'){?>
	<a href="javascript:;" onclick="popup_register( 'mod_review', '<?php echo $TPL_V1["goodsno"]?>', '<?php echo $TPL_V1["sno"]?>' );"><img src="/shop/data/skin/apple_tree/img/common/btn_modify2.gif" border="0" align="absmiddle"></a>
<?php }?>
<?php if($TPL_V1["authdelete"]=='Y'){?>
	<a href="javascript:;" onclick="popup_register( 'del_review', '<?php echo $TPL_V1["goodsno"]?>', '<?php echo $TPL_V1["sno"]?>' );"><img src="/shop/data/skin/apple_tree/img/common/btn_delete.gif" border="0" align="absmiddle"></a>
<?php }?>
</div>
</div>
<?php }}?>
