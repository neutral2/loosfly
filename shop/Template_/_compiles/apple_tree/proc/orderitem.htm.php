<?php /* Template_ 2.2.7 2012/11/14 15:07:40 /www/loosfly_godo_co_kr/shop/data/skin/apple_tree/proc/orderitem.htm 000005987 */ ?>
<style media="screen">
...
<?php }?>
	...
	<div style="margin:5px 0 5px 0;overflow:hidden;height:1px;background:url(/shop/data/skin/apple_tree/img/common/line2.gif) repeat-x top left;"></div>
<?php if($TPL_V1["opt"]){?>선택옵션 : [<?php echo implode("/",$TPL_V1["opt"])?>]<?php }?>
<?php if($TPL_V1["addopt"]){?>
	추가 옵션 :<?php if(is_array($TPL_R2=$TPL_V1["addopt"])&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?>[<?php echo $TPL_V2["optnm"]?>:<?php echo $TPL_V2["opt"]?>]<?php }}?>
<?php }?>
<?php if($GLOBALS["orderitem_mode"]=="cart"){?>
	<div style="margin:5px 0 5px 0"><a href="javascript:void(0);" onClick="nsGodo_CartAction.editOption('<?php echo $TPL_I1?>');"><img src="/shop/data/skin/apple_tree/img/common/btn_check_modify.gif"></a></div>
<?php }?>
	</td>
...
<?php if($GLOBALS["orderitem_mode"]=="cart"){?>
		...
		<tr>
			...
			<td><div style="padding-bottom:2px"><img src="/shop/data/skin/apple_tree/img/common/btn_plus.gif" onClick="chg_cart_ea(frmCart['ea[]'],'up',<?php echo $TPL_I1?>)" style="cursor:pointer"></div><img src="/shop/data/skin/apple_tree/img/common/btn_minus.gif" onClick="chg_cart_ea(frmCart['ea[]'],'dn',<?php echo $TPL_I1?>)" style="cursor:pointer"></td>
			<td><input type=image src="/shop/data/skin/apple_tree/img/common/sbtn_mod.gif"></td>
		</tr>
		</table>
<?php }else{?>
