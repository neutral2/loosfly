...
function popup(src,width,height,stl)
{
	if( stl == '' ) stl = ',scrollbars=1';
	window.open(src,'','width='+width+',height='+height+stl);
}
		...
		gd_ajax({
			...
			success : function(transport){
				...
				if (code=='chkOpenYn'){	//진열여부 체크
					...
					centerLayer(dirPath+'goods/popup_cart_add.php?preview='+preview,350,300);
...
function centerLayer(s,w,h,sw)
{
	...
	with (obj.style){
		...
		backgroundColor = "#ffffff";
 		if( sw < 2 ) border = "1px solid #3f3f3f";
		marginLeft='0px';
		padding='0px';
		zIndex='100';
	}
...
{
	parent.document.body.removeChild(parent.document.getElementById('popLayer') );
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function overS(tag, element) { menu_action(true, tag, element, "font-size:11px;"); };
	function outS(tag, element) { menu_action(false, tag, element, "font-size:12px;"); };

	function overM(tag, element) { menu_action(true, tag, element, "font-size:12px;"); };
	function outM(tag, element) { menu_action(false, tag, element, "font-size:13px;"); };	
	function overM_1(tag, element) { menu_action(true, tag, element, "font-size:12px; color:#f47d30"); };
	function outM_1(tag, element) { menu_action(false, tag, element, "font-size:13px; color:#f47d30;"); };

	function overL(tag, element) { menu_action(true, tag, element, "font-size:13px;"); };
	function outL(tag, element) { menu_action(false, tag, element, "font-size:14px;"); };	

	function menu_action(isOver, tag, element, cssText) {
		element.style.cssText = cssText;
		if( isOver ) 
			element.style.cssText += "font-family:\"나눔고딕\", \"Nanum Gothic\", \"AppleGothic\", \"맑은고딕\", \"Malgun Gothic\", \"돋움\", \"Dotum\", \"Sans-serif\";";
		else 
			element.style.cssText += "font-family:\"Courier New\", Courier, monospace;";
	...
	function commentLoginAlert(element) {
		element.value="";
		alert("로그인을 하셔야 댓글을 남기실 수 있습니다.");
...
//	form.checksaveid.checked = ((form.password.value = getCookie("savepass")) != "");
}
...
function reveal( sw ) {
	document.getElementById("estro").style.display = "none";
	document.getElementById("estro").style.height = "0px";
	document.getElementById("shirring").style.display = "none";
	document.getElementById("shirring").style.height = "0px";

	if( sw == 1 ) { 
		document.getElementById("estro").style.display = "";
		document.getElementById("estro").style.height = "115px";
	}
	else if( sw == 2 ) {
		document.getElementById("shirring").style.display = "";
		document.getElementById("estro").style.height = "115px";
	}
}
