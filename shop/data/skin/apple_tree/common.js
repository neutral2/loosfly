...
function createXMLFromString(string) {
   ...
   return xmlDocument;
		...
		var block04start = 0;
		var max_block04_img = 8;
		var features = [
			{ "img": "test/img/model_01.jpg", "label" : "model 1" },
			{ "img": "test/img/model_02.jpg", "label" : "model 2" },
			{ "img": "test/img/model_03.jpg", "label" : "model 3" },
			{ "img": "test/img/model_04.jpg", "label" : "model 4" },
			{ "img": "test/img/model_05.jpg", "label" : "model 5" },
			{ "img": "test/img/model_06.jpg", "label" : "model 6" },
			{ "img": "test/img/model_07.jpg", "label" : "model 7" },
			{ "img": "test/img/model_08.jpg", "label" : "model 8" }
		];
		...
		$(document).ready(function() {
			...
			$slid = $('#search_label');
			$siid.focus( function() {
				$slid.css("visibility", "hidden");
			});
			$siid.blur( function() {
				$this = $(this);
				if( $this.attr("id") == "search_input" && $this.attr('value') == "" )
				$slid.css("visibility", "visible");
			});

			block04roll('next');
		});
		
		block04roll=function(direction) {
			if( direction == 'back' ) block04start--;
			else if( direction == 'next' ) block04start++;
						
			if( block04start < 1 ) block04start = max_block04_img;
			else if( block04start > max_block04_img ) block04start = 1;

			nstart = block04start;
			for( i = 0; i < 4; i++ ) {
				img_id = "block04_img" + String(i+1);
				label_id = "block04_label" + String(i+1);

				document.getElementById(img_id).style.backgroundImage = "url("+features[nstart-1].img+")";
				document.getElementById(label_id).innerHTML = "<font color=\"white\">"+features[nstart-1].label+"</font>";
				if( ++nstart > max_block04_img ) nstart = 1;
			}
		}
